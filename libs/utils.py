...
def interp(l, r, n_samples):
    ...
    """
    return np.array([
        l + step_i / (n_samples - 1) * (r - l) for step_i in range(n_samples)
    ])
...
def slice_montage(montage, img_h, img_w, n_imgs):
    ...
    for i in range(int(np.sqrt(n_imgs))):
        for j in range(int(np.sqrt(n_imgs))):
            sliced_ds.append(
                montage[1 + i + i * img_h:1 + i + (i + 1) * img_h, 1 + j +
                        j * img_w:1 + j + (j + 1) * img_w])
    ...
    for i in range(n_plots):
        for j in range(n_plots):
            ...
            if this_filter < images.shape[0]:
                ...
                m[1 + i + i * img_h:1 + i + (i + 1) * img_h, 1 + j +
                  j * img_w:1 + j + (j + 1) * img_w] = this_img
    ...
    for i in range(n_plots):
        for j in range(n_plots):
            ...
            if this_filter < W.shape[-1]:
                m[1 + i + i * W.shape[0]:1 + i + (i + 1) * W.shape[0], 1 + j +
                  j * W.shape[1]:1 + j + (j + 1) * W.shape[1]] = (np.squeeze(
                      W[:, :, :, this_filter]))
    return m
...
def get_celeb_files(dst='img_align_celeba', max_images=100):
    ...
    files = [
        os.path.join(dst, file_i) for file_i in os.listdir(dst)
        if '.jpg' in file_i
    ][:max_images]
    return files
...
def gauss(mean, stddev, ksize):
    ...
    with tf.Session(graph=g):
        ...
        z = (tf.exp(
            tf.negative(tf.pow(x - mean, 2.0) / (2.0 * tf.pow(stddev, 2.0)))) *
    ...
    def zipdir(path, zf):
        for root, dirs, files in os.walk(path):
            for file in files:
                ...
                if file.endswith(file_list) or file.endswith(
                        optional_file_list):
...
def normalize(a, s=0.1):
    ...
    return np.uint8(
        np.clip((a - a.mean()) / max(a.std(), 1e-4) * s + 0.5, 0, 1) * 255)
...
def conv2d_squeeze(x,
    ...
    with tf.variable_scope(name or 'conv2d_squeeze', reuse=reuse):
        ...
        fire_e1 = tf.nn.conv2d(
            fire, W_e1, strides=[1, d_h, d_w, 1], padding=padding)
        fire_e3 = tf.nn.conv2d(
            fire, W_e3, strides=[1, d_h, d_w, 1], padding=padding)
...
def deconv2d_squeeze(x,
    ...
    with tf.variable_scope(name or 'deconv2d_squeeze', reuse=reuse):
        ...
        W_e1 = tf.get_variable(
            ...
            shape=[
                1, 1,
                int(n_output_ch / ratio) + 1, n_input_ch or x.get_shape()[-1]
            ],
            initializer=tf.contrib.layers.xavier_initializer_conv2d())
        W_e3 = tf.get_variable(
            name='W_e3',
        ...
        fire_e1 = tf.nn.conv2d_transpose(
            ...
            filter=W_e1,
            output_shape=tf.stack([
                tf.shape(x)[0], n_output_h, n_output_w,
                int(n_output_ch / ratio) + 1
            ]),
            strides=[1, d_h, d_w, 1],
            padding=padding)
...
def to_tensor(x):
    ...
    if len(x.get_shape()) == 2:
        ...
        if x_dim == int(x_dim):
            x_dim = int(x_dim)
            x_tensor = tf.reshape(x, [-1, x_dim, x_dim, 1], name='reshape')
        elif np.sqrt(n_input / 3) == int(np.sqrt(n_input / 3)):
            x_dim = int(np.sqrt(n_input / 3))
            x_tensor = tf.reshape(x, [-1, x_dim, x_dim, 3], name='reshape')
        else:
            x_tensor = tf.reshape(x, [-1, 1, 1, n_input], name='reshape')
