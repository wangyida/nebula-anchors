...
def create_input_pipeline(files,
    ...
    if input_shape[0] > input_shape[1]:
        rsz_shape = [
            int(input_shape[0] / input_shape[1] * crop_shape[0] / crop_factor),
    ...
    producer = tf.train.string_input_producer([files],
                                              shuffle=False,
                                              seed=seed)
...
class DatasetSplit(object):
    ...
    def next_batch(self, batch_size=100):
        ...
        while self.current_batch_idx < len(self.images):
            end_idx = min(self.current_batch_idx + batch_size,
                          len(self.images))
            this_batch = {
                'images':
...
class Dataset(object):
    ...
    def __init__(self, Xs, ys=None, split=[1.0, 0.0, 0.0], one_hot=False):
        ...
        self.valid_idxs = idxs[len(self.train_idxs):len(self.train_idxs) +
                               round(split[1] * n_idxs)]
